...
public class SimulationServer extends VisServer {
	...
	public SimulationServer(int port) throws IOException, InterruptedException {
		...
		if (SimulationConfig.isServerConfig()) {
			try {
				this.simulations.add(new GitSimulationHandle("meisser", "course18", "master", true));
				this.simulations.add(new GitSimulationHandle("meisser", "course18", "ex1-hermit-1", true));
